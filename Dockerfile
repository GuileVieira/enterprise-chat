...
RUN touch .env && \
    ...
    attempt=1 && \
    until timeout "$NPM_CI_TIMEOUT_SECONDS" npm ci --include=dev --no-audit; do \
        status=$?; \
        if [ "$attempt" -ge "$NPM_CI_ATTEMPTS" ]; then \
            exit "$status"; \
        fi; \
        echo "npm ci --include=dev --no-audit failed with exit code $status; retrying attempt $((attempt + 1))/$NPM_CI_ATTEMPTS"; \
        attempt=$((attempt + 1)); \
