...
export function createConversationMethods(
  ...
  async function bulkSaveConvos(conversations: Array<Record<string, unknown>>) {
    try {
      ...
      const bulkOps = conversations.map((convo) => {
        ...
        delete sanitized.chatProjectId;
  ...
  async function getConvosByCursor(
    ...
    if (search) {
      try {
        const meiliFilters = [`user = "${escapeMeiliFilterValue(user)}"`];
        if (projectId === 'unassigned') {
          meiliFilters.push('projectId IS NULL');
        } else if (projectId) {
          meiliFilters.push(`projectId = "${escapeMeiliFilterValue(projectId)}"`);
        }
    ...
    try {
      ...
      const convos = await Conversation.find(query)
        ...
        .select(
          'conversationId endpoint title createdAt updatedAt archivedAt isArchived user model agent_id assistant_id spec iconURL projectId pinned',
        )
        .sort(sortObj)
        .limit(pageSize + 1)
        .lean<IConversation[]>();
  ...
  async function deleteConvos(
    ...
    try {
      ...
      type DeletionConversation = Pick<IConversation, 'conversationId' | 'tenantId' | 'tags'>;
      ...
      let conversations = await Conversation.find(userFilter)
        .select('conversationId tenantId tags')
  ...
  async function archiveAllConvos(user: string) {
    ...
    try {
      ...
      while (true) {
        const idRange: { $lte: Types.ObjectId; $gt?: Types.ObjectId } = lastConversationId
          ? { $gt: lastConversationId, $lte: snapshotBoundary._id }
          : { $lte: snapshotBoundary._id };
        const conversations: Array<Pick<IConversation, '_id'>> = await Conversation.find({
          ...filter,
          _id: idRange,
        })
          .select('_id')
          ...
          .lean<Array<Pick<IConversation, '_id'>>>();
