...
export function createUserGroupMethods(
  ...
  searchPrincipals: (
    ...
    optionsOrSession?: { tenantId?: string; global?: boolean } | ClientSession,
    session?: ClientSession,
  ...
  async function searchPrincipals(
    ...
    const fourthArgIsSession = 'inTransaction' in optionsOrSession;
    const options = fourthArgIsSession ? {} : optionsOrSession;
    const activeSession = fourthArgIsSession ? optionsOrSession : session;
    ...
    if (!typeFilter || typeFilter.includes(PrincipalType.USER)) {
      ...
      if (activeSession) {
        userQuery.session(activeSession);
      }
    ...
    if (!typeFilter || typeFilter.includes(PrincipalType.GROUP)) {
      ...
      if (activeSession) {
        groupQuery.session(activeSession);
      }
    ...
    if (!typeFilter || typeFilter.includes(PrincipalType.ROLE)) {
      ...
      if (Role) {
        ...
        if (activeSession) {
          roleQuery.session(activeSession);
        }
