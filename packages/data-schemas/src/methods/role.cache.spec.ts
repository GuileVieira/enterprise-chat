...
describe('getRoleByName cache is scoped to the active tenant', () => {
  it('uses global system roles and tenant-specific custom roles', async () => {
    await runAsSystem(() =>
      new Role({
        name: 'USER',
        permissions: { [PermissionTypes.PROMPTS]: { [Permissions.USE]: true } },
      }).save(),
    );
    await seedRole(TENANT_A, true);
    await seedRole(TENANT_B, false);

    const [systemRole, roleA, roleB] = await Promise.all([
      runAs(TENANT_A, () => getRoleByName('USER')),
      runAs(TENANT_A, () => getRoleByName(ROLE_NAME)),
      runAs(TENANT_B, () => getRoleByName(ROLE_NAME)),
    ]);

    expect(systemRole.tenantId).toBeUndefined();
  ...
  it('does not expose a tenant custom role when no tenant context is active', async () => {
