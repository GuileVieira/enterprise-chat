...
describe('dark dialog surface', () => {
  it('matches the legacy rendered background in CSS and the runtime theme', () => {
    ...
    expect(appStyles).toMatch(/--surface-dialog:\s*32 32 32;/);
    expect(darkTheme['rgb-surface-dialog']).toBe('32 32 32');
  });
});
...
describe('dark hover surface', () => {
  it('uses the gray-650 midpoint in both CSS and the runtime theme', () => {
    ...
    expect(appStyles).toMatch(/--surface-hover:\s*40 40 40;/);
    expect(darkTheme['rgb-surface-hover']).toBe('40 40 40');
  });
});
...
describe('dark destructive text', () => {
  it('uses red-400 without changing the status error token', () => {
    ...
    expect(appStyles).toMatch(/--text-destructive:\s*248 113 113;/);
...
describe('light brand text', () => {
  it('meets WCAG AA on every light canvas where it is used as text', () => {
    expect(
      belowAA(
        defaultTheme,
        ['rgb-brand-purple'],
