...
export const sharedArtifactMessages = (shareId: string, artifactId: string) =>
  `${shareMessages(shareId)}/artifact/${encodeURIComponent(artifactId)}`;
...
export const projectMeetingInsights = (id: string, meetingId: string) =>
  `${projectMeeting(id, meetingId)}/insights`;
...
export const tenantApiKeys = (tenantId: string, suffix = '') =>
  `${BASE_URL}/api/tenant-api-keys${suffix}?tenantId=${encodeURIComponent(tenantId)}`;
