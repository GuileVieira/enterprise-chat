...
const DEFAULT_OPENROUTER_GEMINI_IMAGE_GEN_DESCRIPTION =
  `Premium Image Generator creates high-quality images through OpenRouter using Nano Banana Pro (google/gemini-3-pro-image-preview), with optional image context. This tool costs 3x tokens.
...
const getOpenRouterGeminiImageGenDescription = (): string => {
  return (
...
export const openRouterGeminiToolkit: Record<
  string,
  {
    name: string;
    description: string;
...
> = {
  openrouter_gemini_image_gen: {
    name: 'openrouter_gemini_image_gen' as const,
    description: getOpenRouterGeminiImageGenDescription(),
...
export type OpenRouterGeminiToolkit = typeof openRouterGeminiToolkit;
