...
interface ImageArtifact {
  content?: Array<{ type?: string; image_url?: string | { url?: string } }>;
  file_ids?: string[];
  session_id?: string;
  files?: Array<{
    id: string;
...
async function primeImageArtifactForCode(
  ...
  for (let index = 0; index < (imageArtifact.content?.length ?? 0); index++) {
    const part = imageArtifact.content?.[index];
    const url = typeof part?.image_url === 'string' ? part.image_url : part?.image_url?.url;
    const match = url?.match(/^data:(image\/(?:png|jpeg|gif|webp));base64,([A-Za-z0-9+/=]+)$/);
    if (!match) continue;
    const id = imageArtifact.file_ids?.[index] ?? `generated-${index + 1}`;
    uploads.push({
                ...
                const execute = async (
                  ...
                  try {
                    ...
                    const artifact = await primeImageArtifactForCode(
                      ...
                      options,
                      mergedConfigurable?.codeEnvAvailable === true,
                      getCodeExecutionContext(mergedConfigurable),
                      mergedConfigurable?.req as ServerRequest | undefined,
                    );
                    ...
                    return {
                      toolCallId: tc.id,
                      content: modelContent as string | unknown[],
                      artifact,
                      status: 'success' as const,
                    };
                  } catch (toolError) {
