...
export async function initializeAgent(
  ...
  if (hasProjectFiles) {
    ...
    for (const resourceType of [EToolResources.file_search, EToolResources.context] as const) {
      if (resourceType === EToolResources.context && !toolResourceSet.has(resourceType)) {
        continue;
      }
      const resource = runtimeToolResources[resourceType] ?? {};
      runtimeToolResources[resourceType] = {
        ...resource,
        file_ids: [...new Set([...(resource.file_ids ?? []), ...projectFileIds])],
      };
    }
  }
  ...
  const tool_resources = primedToolResources ?? runtimeToolResources;
  ...
  const configuredTrafficAgentId = params.req?.config?.interfaceConfig?.metaAdsTrafficAgentId;
