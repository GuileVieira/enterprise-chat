    ...
    it('uploads generated image bytes and returns a code-session artifact', async () => {
      ...
      const [result] = await invokeHandlerWithConfig(
        ...
        [{ id: 'image-call', name: 'image_gen_oai', args: { prompt: 'chart' } }],
        {
          ...
          codeExecutionContext: {
            ...
            codeSessionKey: 'session-key',
            executionProfile: 'stateful',
            executionRouteKey: 'agent-route',
            runtimeSessionHint: 'runtime-hint',
      ...
      expect(batchUploadCodeEnvFiles).toHaveBeenCalledWith(
        expect.objectContaining({
          kind: 'user',
          id: 'user-1',
          codeApiBaseUrl: 'https://code.example',
          executionProfile: 'stateful',
          executionRouteKey: 'agent-route',
          runtime_session_hint: 'runtime-hint',
          bridgeWorkerId: 'worker-1',
...
describe('createToolExecuteHandler tenant function compatibility', () => {
  it('uses DynamicStructuredTool ToolMessage content as event-driven result content', async () => {
    ...
    const handler = createToolExecuteHandler({ loadTools });
    const results = await invokeHandler(handler, [
      { id: 'call_tenant', name: 'tenant_report', args: { accountId: 'act_1' } },
    ]);
