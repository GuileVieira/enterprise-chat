...
describe('createPrincipalSearch', () => {
  ...
  const setupSearch = ({
    ...
    const req = {
      ...
      user: { id: 'user123', role, tenantId: tenantId ?? undefined, openidId: 'oid-1' },
  ...
  it.each([SystemRoles.OWNER, SystemRoles.USER])(
    'scopes %s principal searches to the caller tenant',
    async (role) => {
      const test = setupSearch({ types: [USER, GROUP], role, tenantId: 'tenant-b' });
      await test.run();

      expect(test.deps.searchPrincipals).toHaveBeenCalledWith('alice', 20, [USER, GROUP], {
        tenantId: 'tenant-b',
        global: false,
      });
    },
  ...
  it('allows only system admins to search globally', async () => {
    const test = setupSearch({ types: [USER, GROUP], role: SystemRoles.ADMIN });
    await test.run();
  ...
  it('keeps a tenantless member search non-global so the data helper fails closed', async () => {
    const test = setupSearch({ types: [USER, GROUP], tenantId: null });
    await test.run();
