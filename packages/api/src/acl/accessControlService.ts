...
export class AccessControlService {
  ...
  public async bulkUpdateResourcePermissions({
    ...
    try {
      ...
      const bulkWriteTenantIds: Array<string | undefined> = [];
      ...
      const queryPrincipalId = (principal: BulkPrincipal) => {
        if (principal.type === PrincipalType.ROLE || principal.type === PrincipalType.TENANT) {
        ...
        try {
          ...
          const update = {
            $set: {
              ...
              ...(principal.type === PrincipalType.TENANT && { tenantId: principal.id }),
              roleId: role._id,
              grantedBy,
              grantedAt,
            },
          ...
          bulkWriteTenantIds.push(
            principal.type === PrincipalType.TENANT ? principal.id?.toString() : undefined,
          );
      ...
      if (bulkWrites.length > 0) {
        const unscopedWriteIndexes = bulkWrites
          .map((_, index) => index)
          .filter((index) => bulkWriteTenantIds[index] == null);
        const unscopedWrites = unscopedWriteIndexes.map((index) => bulkWrites[index]);
        const unscopedIndexByOriginal = new Map(
          unscopedWriteIndexes.map((originalIndex, filteredIndex) => [
            originalIndex,
            filteredIndex,
          ]),
        );
        const bulkWriteResult: Awaited<ReturnType<AllMethods['bulkWriteAclEntries']>> | undefined =
          unscopedWrites.length > 0
            ? await this._dbMethods.bulkWriteAclEntries(unscopedWrites, sessionOptions)
            : undefined;
        const tenantIds = new Set(bulkWriteTenantIds.filter((id): id is string => id != null));
        for (const tenantId of tenantIds) {
          const tenantWrites = bulkWrites.filter(
            (_, index) => bulkWriteTenantIds[index] === tenantId,
          );
          const currentContext = tenantStorage.getStore() ?? {};
          await tenantStorage.run(
            { ...currentContext, tenantId },
            async () => await this._dbMethods.bulkWriteAclEntries(tenantWrites, sessionOptions),
          );
        }
        for (const [writeIndex, change] of insightsChangesByBulkWriteIndex) {
          const filteredIndex = unscopedIndexByOriginal.get(writeIndex);
          if (
            filteredIndex != null &&
            bulkWriteResult?.upsertedIds?.[filteredIndex] &&
      ...
      for (const principal of revokedPrincipals) {
        try {
          ...
          const deleteEntry = () => this._dbMethods.deleteAclEntries(query, sessionOptions);
          const deleteResult =
            principal.type === PrincipalType.TENANT && principal.id != null
              ? await tenantStorage.run(
                  { ...(tenantStorage.getStore() ?? {}), tenantId: principal.id.toString() },
                  async () => await deleteEntry(),
                )
              : await deleteEntry();
          const existingEntry = currentEntriesByPrincipal.get(
            `${principal.type}:${principal.type === PrincipalType.PUBLIC ? 'null' : principal.id}`,
