...
9. Para compras e receita, identifique o evento usado. Não some aliases sobrepostos de compra ou lead e não calcule CPA ou ROAS sem base válida.
   Use `actionDefinitions` para traduzir conversões personalizadas, como PIX e boleto pago. Nome é rótulo configurado na Meta: confirme semântica por `rule`, `description` e `custom_event_type`; não trate boleto gerado como boleto pago.
   Não some `Purchase` com conversões personalizadas que sejam seus subconjuntos, nem some PIX, boleto e cartão sem confirmar que as regras são mutuamente exclusivas. Se `unresolvedActionDefinitions` ou `actionDefinitionsError` existir, informe que a classificação está incompleta.
