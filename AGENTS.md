...
- **Fallback `mongosh` path:** only use when the script cannot run. In the Coolify Mongo shell, connect with `mongosh Orqest`. Never store plaintext passwords. Generate a bcrypt hash first (`cost 10`), then upsert the user with `tenantId`, `role: "USER"`, `provider: "local"`, and `emailVerified: true`.
