...
describe('archived conversation mutation cache reconciliation', () => {
  ...
  it('invalidates list and project caches after moving an archived conversation', async () => {
    const queryClient = createQueryClient();
    const pinnedKey = [QueryKeys.pinnedConversations, { tags: [] }];
    const oldProjectKey = [QueryKeys.project, 'project-old'];
    const newProjectKey = [QueryKeys.project, 'project-new'];
    queryClient.setQueryData([QueryKeys.conversation, archivedConversation.conversationId], {
      ...archivedConversation,
      projectId: 'project-old',
    });
    queryClient.setQueryData(archivedKey, { pages: [], pageParams: [] });
    queryClient.setQueryData(pinnedKey, { conversations: [] });
    queryClient.setQueryData(oldProjectKey, { id: 'project-old' });
    queryClient.setQueryData(newProjectKey, { id: 'project-new' });
    updateConversation.mockResolvedValue({
      ...archivedConversation,
      projectId: 'project-new',
    });

    const { result } = renderHook(() => useMoveConversationToProjectMutation(), {
      wrapper: createWrapper(queryClient),
    });
    ...
    expect(queryClient.getQueryState(oldProjectKey)?.isInvalidated).toBe(true);
    expect(queryClient.getQueryState(newProjectKey)?.isInvalidated).toBe(true);
  });
