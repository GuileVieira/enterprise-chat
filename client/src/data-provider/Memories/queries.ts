...
export const useUpdateMemoryMutation = (
  ...
  const queryClient = useQueryClient();
  return useMutation(
    ({ key, id, value, originalKey, agentId, expectedUpdatedAt }: UpdateMemoryParams) => {
      if (id) {
        return dataService.updateMemoryById(id, value, key, agentId, expectedUpdatedAt);
      }
      if (key) {
        return dataService.updateMemory(key, value, originalKey, expectedUpdatedAt, agentId);
      }
      throw new Error('Memory address is required.');
    },
    {
