  ...
  return useMutation(
      ...
      dataService.updateConversation({ conversationId, projectId }),
    {
      onSuccess: (updatedConvo) => {
        ...
        const previous = queryClient.getQueryData<t.TConversation>([
        ...
        for (const projectId of new Set([previous?.projectId, updatedConvo.projectId])) {
          if (projectId) {
  ...
  return useMutation(
    (payload: t.TArchiveConversationRequest) => dataService.archiveConversation(payload),
    {
      ...
      onSuccess: (_data, vars, context) => {
        ...
        if (_data.projectId) {
          queryClient.invalidateQueries([QueryKeys.project, _data.projectId]);
        }
      ...
      dataService.deleteConversation(payload) as Promise<t.TDeleteConversationResponse>,
    {
      ...
      onSuccess: (data, vars, context) => {
        const deletedConversation = vars.conversationId
          ...
          : undefined;
        let deletedProjectId = deletedConversation?.projectId;
        if (!deletedProjectId && vars.conversationId) {
          /* An archived row can be the only cached copy carrying `projectId`. */
          const cacheKeys = [QueryKeys.allConversations, QueryKeys.archivedConversations];
          for (const cacheKey of cacheKeys) {
            ...
            for (const query of queries) {
              ...
              if (found?.projectId) {
                deletedProjectId = found.projectId;
                break;
              }
            }
            if (deletedProjectId) {
         ...
         * `projectId` so the Orqest project view can drop its stale count. */
        if (!deletedProjectId && vars.conversationId) {
...
export const useDuplicateConversationMutation = (
  ...
  return useMutation((payload) => dataService.duplicateConversation(payload), {
    onSuccess: (data, vars, context) => {
      ...
      queryClient.invalidateQueries([QueryKeys.pinnedConversations]);
      queryClient.invalidateQueries([QueryKeys.projects]);
      if (duplicatedConversation.projectId) {
        queryClient.invalidateQueries([QueryKeys.project, duplicatedConversation.projectId]);
      }
...
export const useForkConvoMutation = (
  ...
  return useMutation((payload: t.TForkConvoRequest) => dataService.forkConversation(payload), {
    onSuccess: (data, vars, context) => {
      ...
      queryClient.invalidateQueries([QueryKeys.pinnedConversations]);
      queryClient.invalidateQueries([QueryKeys.projects]);
      if (forkedConversation.projectId) {
        queryClient.invalidateQueries([QueryKeys.project, forkedConversation.projectId]);
      }
