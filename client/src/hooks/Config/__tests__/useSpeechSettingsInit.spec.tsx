...
describe('useSpeechSettingsInit', () => {
  ...
  it('applies admin speech disables despite saved enabled preferences', async () => {
    localStorage.setItem('speechToText', JSON.stringify(true));
    localStorage.setItem('textToSpeech', JSON.stringify(true));
    mockUseGetCustomConfigSpeechQuery.mockReturnValue({
      data: { sttExternal: true, ttsExternal: true, speechToText: false, textToSpeech: false },
      isFetched: true,
    });
    ...
    await waitFor(() => {
      ...
      expect(result.current.textToSpeech).toBe(false);
    });
  });
