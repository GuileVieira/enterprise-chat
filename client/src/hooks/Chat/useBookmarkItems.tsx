...
import { Constants, PermissionTypes, Permissions, QueryKeys } from 'librechat-data-provider';
...
export default function useBookmarkItems({
  ...
  const { data } = useConversationTagsQuery({ enabled: enabled && canUseBookmarks });
  ...
  return {
    show: enabled && canUseBookmarks && isActiveConvo && !isTemporary,
    items,
    bookmarks: data ?? [],
