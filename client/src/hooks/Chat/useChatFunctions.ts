  ...
  const ask: TAskFunction = (
    ...
    const projectId =
      conversationId === Constants.NEW_CONVO
        ? (getRouteProjectId() ?? conversation?.projectId ?? null)
        : (conversation?.projectId ?? null);
    const conversationForPayload =
      projectId != null ? { ...(conversation ?? {}), projectId } : (conversation ?? {});
    ...
    if (conversationId == Constants.NEW_CONVO) {
      ...
      currentMessages = [];
      conversationId = null;
      const projectSearch = projectId ? `?projectId=${encodeURIComponent(projectId)}` : '';
      requestChatFocus();
