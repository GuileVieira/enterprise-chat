...
jest.mock('~/data-provider', () => ({
  startupConfigKey: (enabled: boolean) => ['startupConfig', enabled],
}));
jest.mock('~/store', () => ({
  __esModule: true,
  default: {
...
describe('useChatFunctions ask', () => {
  ...
  it('keeps hidden prompt content in the submission without exposing it in message metadata', () => {
    const hiddenPromptContext = {
      promptGroupId: 'group-1',
      promptId: 'prompt-1',
      name: 'Project DNA',
      description: 'Private project guidance',
      content: 'SECRET MODEL-ONLY INSTRUCTIONS',
    };
    const { result, setSubmission } = renderAsk([]);
    ...
    const submission = setSubmission.mock.calls.at(-1)?.[0] as TSubmission;
    expect(submission.hiddenPromptContext).toEqual(hiddenPromptContext);
    expect(submission.initialResponse).not.toHaveProperty('hiddenPromptContext');
    expect(submission.initialResponse?.metadata).toEqual({ responseAvatarLabel: 'agent-1' });
    expect(submission.userMessage.metadata).toEqual({
      hiddenPrompt: {
        name: 'Project DNA',
        promptGroupId: 'group-1',
        description: 'Private project guidance',
    ...
    expect(JSON.stringify(submission.userMessage.metadata)).not.toContain(
      'SECRET MODEL-ONLY INSTRUCTIONS',
    );
  });
