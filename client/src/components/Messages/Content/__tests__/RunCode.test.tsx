...
jest.mock('~/data-provider', () => ({
  ...jest.requireActual('~/data-provider/Tools/mutations'),
  useVerifyAgentToolAuth: () => ({ data: { authenticated: true } }),
}));
jest.mock('~/components/SidePanel/Agents/Code/ApiKeyDialog', () => () => null);
    ...
    async (outcome) => {
      ...
      expect(callTool).toHaveBeenCalledWith(
        expect.objectContaining({
          toolParams: expect.objectContaining({ code: 'print(1)', isTemporary: false }),
        }),
      );
