...
function renderComposer({
  ...
  queryClient.setQueryData(
    ...
    disableSpeech ? { speechToText: false, textToSpeech: false } : {},
  );
    ...
    <QueryClientProvider client={queryClient}>
      <RecoilRoot
        initializeState={({ set }) => {
          ...
          set(
            store.activeHiddenPromptByIndex(0),
            hiddenPrompt
              ? {
...
describe('ChatForm attachments', () => {
  ...
  test('shows and removes the active hidden prompt without submitting the form', async () => {
    renderComposer({ hiddenPrompt: true });

    const badge = await screen.findByText(/Project DNA/);
    expect(badge).toBeInTheDocument();
    await userEvent.click(badge.parentElement?.querySelector('button') as HTMLButtonElement);

    expect(screen.queryByText(/Project DNA/)).not.toBeInTheDocument();
  });

  test('admin speech settings override enabled local speech preferences', async () => {
    renderComposer({ disableSpeech: true });
