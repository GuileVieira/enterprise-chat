...
const MessageRender = memo(function MessageRender({
  ...
  const iconData: TMessageIcon = useMemo(
    () => {
      const responseAvatarLabel = msg?.metadata?.responseAvatarLabel;
      let avatarLabel: string | undefined;
      if (typeof responseAvatarLabel === 'string') {
        avatarLabel = responseAvatarLabel;
      } else if (msg?.isCreatedByUser === false) {
        avatarLabel = messageLabel;
      }
    ...
    // `sender` changes the derived sender label in useMessageActions.
