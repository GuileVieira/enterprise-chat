      ...
      <div ref={contentRef} className="flex max-w-4xl flex-col items-center gap-0 p-2">
        ...
        >
          ...
          {((isAgent || isAssistant) && name) || name ? (
            <div className="flex max-w-4xl flex-col items-center gap-0 p-2">
              <SplitText
                key={`split-text-${name}`}
                text={name}
        ...
        {description &&
          (descriptionIsHTML ? (
            <div
              className="animate-fadeIn mt-4 flex max-w-xl items-center justify-center gap-2 text-pretty text-center text-sm font-normal leading-6 text-text-secondary [&_img]:inline-block [&_img]:h-4 [&_img]:w-4"
