...
export default function GenericGrantAccessDialog({
  ...
  const { data: tenantsData } = useListAdminTenants({ enabled: canSelectTenants && isModalOpen });
  ...
  const handleAddTenant = () => {
    if (
      !selectedTenantId ||
      allShares.some((share) => principalKey(share) === `tenant-${selectedTenantId}`)
    )
      return;
    setAllShares((shares) => [
      ...shares,
                  ...
                  <>
                    ...
                    {canSelectTenants && (tenantsData?.tenants.length ?? 0) > 0 && (
                      <div className="flex items-center gap-2">
                        ...
                        <select
                          className="h-10 min-w-0 flex-1 rounded-md border border-border-medium bg-surface-primary px-3 text-sm"
            ...
            {resourceId && resourceUrl && (
              <TooltipAnchor
                ...
                render={
                  ...
                  >
                    {isCopying ? <Check className="size-4" /> : <Link className="size-4" />}
                    {isCopying
