...
const mockUseConversationsInfiniteQuery = jest.fn((_params?: unknown) => mockConversationsResult);
...
jest.mock('~/store', () => {
  ...
  return {
    ...
    default: {
      ...
      selectedProjectId: recoilAtom({ key: 'mock-cs-selectedProjectId', default: 'orqest-1' }),
      search: recoilAtom({
        key: 'mock-cs-search',
...
jest.mock('~/data-provider', () => ({
  __esModule: true,
  useConversationsInfiniteQuery: (params: unknown) => mockUseConversationsInfiniteQuery(params),
...
jest.mock('~/utils', () => ({
  collectPinnedConversations: (pinned: unknown[] = []) => pinned,
...
jest.mock('~/components/Conversations/ChatFilterMenu', () => ({
  __esModule: true,
  default: () => <div data-testid="chat-filter-stub" />,
}));
...
describe('ConversationsSection section order', () => {
  it('renders the Orqest selector before Pinned and Chats', async () => {
    const { getByTestId } = renderSection();
    await settleRenders();

    const projects = getByTestId('project-selector-stub');
  ...
  it('passes the selected Orqest project only as a conversation-list filter', async () => {
    renderSection();
    await settleRenders();

    expect(mockUseConversationsInfiniteQuery).toHaveBeenCalledWith(
      expect.objectContaining({ projectId: 'orqest-1' }),
    );
  });
});
