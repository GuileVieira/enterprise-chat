...
let mockSharedLinkTarget: string | undefined;
let mockTenantOnSuccess:
  | ((
      data: { shareId: string },
      variables: { conversationId: string; targetMessageId?: string },
    ) => void)
  | undefined;
const mockCreateTenantLink = jest.fn();
...
jest.mock('librechat-data-provider/react-query', () => ({
  useGetSharedLinkQuery: (_conversationId: string, targetMessageId?: string) => {
    mockSharedLinkTarget = targetMessageId;
    return { data: mockShare, isLoading: false };
  },
}));

jest.mock('~/data-provider', () => ({
  useGetStartupConfig: () => ({ data: { sharedLinksSnapshotFilesEnabled: true } }),
  useCreateTenantSharedLinkMutation: (options: { onSuccess: typeof mockTenantOnSuccess }) => {
    mockTenantOnSuccess = options.onSuccess;
    return { mutate: mockCreateTenantLink, isLoading: false };
  },
}));
...
const renderShareButton = (conversationId = ACTIVE_CONVERSATION_ID, targetMessageId?: string) => {
...
describe('ShareButton', () => {
  beforeEach(() => {
    ...
    mockSharedLinkTarget = undefined;
    mockTenantOnSuccess = undefined;
    mockCreateTenantLink.mockReset();
  });
  ...
  it('preserves an explicit branch target for lookup and publication', async () => {
    mockLatestMessageId = 'foreign-latest';
    mockGetMessageById.mockResolvedValue([{ messageId: 'selected-message' }]);
    renderShareButton(ACTIVE_CONVERSATION_ID, 'selected-message');

    expect(mockSharedLinkTarget).toBe('selected-message');
    expect(screen.getByTestId('share-actions')).toHaveAttribute(
      'data-target-message-id',
      'selected-message',
    );
    await expect(mockResolveTargetMessageId?.()).resolves.toBe('selected-message');
    expect(mockGetMessageById).toHaveBeenCalledWith(ACTIVE_CONVERSATION_ID, 'selected-message');
  });

  it('ignores a late tenant-link response after the conversation changes', () => {
    const { rerender, queryClient } = renderShareButton(ACTIVE_CONVERSATION_ID, 'message-1');
    const staleOnSuccess = mockTenantOnSuccess;
    ...
    staleOnSuccess?.(
      { shareId: 'stale-share' },
      { conversationId: ACTIVE_CONVERSATION_ID, targetMessageId: 'message-1' },
    );

    expect(screen.queryByText(/stale-share/)).not.toBeInTheDocument();
    expect(screen.getByRole('button', { name: 'com_ui_create_tenant_link' })).toBeInTheDocument();
  });
