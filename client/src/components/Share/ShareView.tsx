...
import { ExternalLink, RefreshCw, MessageSquarePlus } from 'lucide-react';
...
import { useForkSharedConvoMutation, useForkTenantShareMutation } from '~/data-provider';
...
function SharedView({ isTenantShare = false }: { isTenantShare?: boolean }) {
  ...
  const isArtifactShare = shareId != null && searchParams.has('artifact');
  const jotaiStore = useStore();
  const { data: sharedConfig } = useGetSharedStartupConfig(shareId, {
    enabled: isAuthReady && !isTenantShare,
  });
  const { data: tenantConfig } = useGetStartupConfig({
    enabled: isAuthReady && isTenantShare,
  });
  const config = isTenantShare ? tenantConfig : sharedConfig;
  const publicShare = useGetSharedMessages(shareId ?? '', {
    enabled: isAuthReady && !isTenantShare && !isArtifactShare,
  });
  const tenantShare = useGetTenantSharedMessages(shareId ?? '', {
    enabled: isAuthReady && isTenantShare && !isArtifactShare,
  });
  const { data, isLoading, isFetching, refetch } = isTenantShare ? tenantShare : publicShare;
  ...
  const handleContinue = useCallback(() => {
    ...
    if (isTenantShare) {
      forkTenantShare.mutate({ shareId });
      return;
    }
    forkSharedConvo({
      shareId,
      ...
      <>
        <ShareHeader
          ...
          continueLabel={localize(
            isTenantShare ? 'com_ui_fork_to_my_account' : 'com_ui_continue_chat',
          )}
          langfuseSessionLabel={localize('com_ui_langfuse_view_session')}
          langfuseSessionUrl={data.langfuseSessionUrl}
          onContinue={handleContinue}
          isContinuing={isTenantShare ? forkTenantShare.isLoading : forkShare.isLoading}
        />
