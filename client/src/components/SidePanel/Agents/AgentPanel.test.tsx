...
describe('AgentPanel - Update Agent Toast Messages', () => {
  ...
  describe('AgentPanel', () => {
    ...
    it('preserves selected tenant function ids in the update payload', async () => {
      const { mockUseGetAgentByIdQuery, mockUpdateAgent } = setupMocks();
      mockAgentQuery(mockUseGetAgentByIdQuery, { name: 'Test Agent', version: 2, tools: [] });
      mockUpdateAgent.mockResolvedValue(
        createMockAgent({ name: 'Test Agent', version: 3, tools: ['tenant_function_1'] }),
      );

      const Wrapper = createWrapper();
      const { container } = render(<AgentPanel />, { wrapper: Wrapper });
      act(() => {
        capturedFormMethods!.setValue('tools', ['tenant_function_1'], { shouldDirty: true });
      });
        ...
        expect(mockUpdateAgent).toHaveBeenCalledWith(
          expect.objectContaining({
            data: expect.objectContaining({
              tools: expect.arrayContaining(['tenant_function_1']),
            }),
          }),
        ),
      );
