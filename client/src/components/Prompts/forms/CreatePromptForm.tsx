...
const CreatePromptForm = ({
  ...
  const onSubmit = (data: CreateFormValues) => {
    if (isCreating || creationPendingRef.current) {
      return;
    }
      ...
      <form onSubmit={handleSubmit(onSubmit)} className={cn('w-full', !isDialog && 'px-4 py-2')}>
        ...
        <div className="mb-1 flex flex-col items-center justify-between font-bold sm:text-xl md:mb-0 md:text-2xl">
          <div className="flex w-full flex-col items-center justify-between sm:flex-row">
            ...
            />
            {!isSmallScreen && <CategorySelector portal={!isDialog} />}
          </div>
        </div>
            ...
            <div className="rounded-lg border border-border-light bg-surface-secondary/40 p-3">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div className="flex min-w-0 gap-2 sm:w-[28rem]">
                  <select
                    className="h-10 min-w-0 flex-1 rounded-md border border-border-medium bg-surface-primary px-3 text-sm text-text-primary outline-none transition-colors hover:border-border-xheavy focus:border-border-xheavy focus:ring-2 focus:ring-ring-primary/20"
                    value={tenantToAdd}
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  {selectedTenantIds.map((tenantId) => (
                    ...
                    >
                      ...
                      <button
                        type="button"
                        className="rounded-sm text-text-secondary transition-colors hover:text-text-primary focus:outline-none focus:ring-2 focus:ring-ring-primary/30"
