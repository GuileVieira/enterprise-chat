...
jest.mock('~/models', () => ({
  ...
  getTenantSharedLinkForFork: jest.fn(),
}));
...
describe('forkConversationFromSource', () => {
  ...
  test('forks tenant shares under viewer retention without owner file IDs or source expiry', async () => {
    const interfaceConfig = { retentionMode: 'all', retention: { days: 7 } };
    ...
    getTenantSharedLinkForFork.mockResolvedValue({
      targetMessageId: 'msg_a',
      conversation: {
        title: 'Tenant Shared Title',
        isTemporary: true,
        expiredAt: new Date('2021-01-03T00:00:00.000Z'),
      },
      messages: [
        {
          messageId: 'msg_a',
          ...
          attachments: [{ file_id: 'owner-file-2', toolCallId: 'tool_1' }],
        },
      ],
    });

    await forkSharedConversation({
      shareId: 'tenant-share',
      requestUserId: 'viewer-user',
      tenantId: 'tenant-a',
      userRole: 'USER',
      loadAppConfig,
      builderFactory,
    ...
    expect(builderFactory).toHaveBeenCalledWith('viewer-user', interfaceConfig, undefined);
    const [savedMessage] = bulkSaveMessages.mock.calls[0][0];
    expect(savedMessage.files[0]).not.toHaveProperty('file_id');
    expect(savedMessage.attachments[0]).not.toHaveProperty('file_id');
    const [savedConversation] = bulkSaveConvos.mock.calls[0][0];
    expect(savedConversation.isTemporary).toBe(false);
    expect(savedConversation).not.toHaveProperty('expiredAt', new Date('2021-01-03T00:00:00.000Z'));
  });

  test('applies viewer content filters before saving a tenant-shared fork', async () => {
    getTenantSharedLinkForFork.mockResolvedValue({
      targetMessageId: 'msg_a',
      conversation: { title: 'Tenant Shared Title' },
      messages: [
        {
          messageId: 'msg_a',
          parentMessageId: Constants.NO_PARENT,
