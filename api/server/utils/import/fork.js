...
async function forkTenantSharedConversation({
  shareId,
  requestUserId,
  tenantId,
  targetMessageId,
  ...
  return tenantStorage.run({ tenantId, userId: requestUserId }, async () => {
    const appConfig = await loadAppConfig({ role: userRole, userId: requestUserId, tenantId });
    ...
    const requestedTarget = targetMessageId || shared.targetMessageId;
    let sourceMessages = shared.messages;
    if (requestedTarget && option === ForkOptions.DIRECT_PATH) {
      sourceMessages = BaseClient.getMessagesForConversation({
        messages: shared.messages,
        parentMessageId: requestedTarget,
      });
    } else if (requestedTarget && option === ForkOptions.INCLUDE_BRANCHES) {
      sourceMessages = getAllMessagesUpToParent(shared.messages, requestedTarget);
    } else if (requestedTarget) {
      sourceMessages = getMessagesUpToTargetLevel(shared.messages, requestedTarget);
    }
    const messagesToClone = sourceMessages.map(({ model: _model, ...message }) =>
      stripSharedFileIds(message),
    );

    cloneMessagesWithTimestamps(messagesToClone, importBatchBuilder);
    const title = await getNextCopyTitle(requestUserId, shared.conversation?.title);
    const result = importBatchBuilder.finishConversation(title, new Date(), {}, model);
    await importBatchBuilder.saveBatch();
 ...
 */
async function forkPublicSharedConversation({
  shareId,
  shareResourceId,
  requestUserId,
  userRole,
...
const forkSharedConversation = (params) =>
  Object.hasOwn(params, 'tenantId')
    ? forkTenantSharedConversation(params)
    : forkPublicSharedConversation(params);
