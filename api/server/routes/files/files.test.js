...
describe('File Routes - Delete with Agent Access', () => {
  ...
  describe('DELETE /files', () => {
    ...
    it('preserves partial deletion details in the HTTP response', async () => {
      const userFileId = uuidv4();
      await createFile({
        user: otherUserId,
        file_id: userFileId,
        filename: 'partial.txt',
        filepath: '/uploads/partial.txt',
        bytes: 200,
        type: 'text/plain',
      });
      processDeleteRequest.mockResolvedValueOnce({
        deletedFileIds: [],
        failedFileIds: [userFileId],
      });

      const response = await request(app)
        .delete('/files')
        .send({ files: [{ file_id: userFileId, filepath: '/uploads/partial.txt' }] });
    ...
    it('denies mixed authorized project and foreign unscoped file deletion', async () => {
      ...
      const response = await request(app)
        ...
        .send({
          projectId,
          files: [
            { file_id: projectFileId, filepath: '/uploads/project.txt' },
            { file_id: foreignFileId, filepath: '/uploads/foreign.txt' },
          ],
        });

      expect(response.status).toBe(403);
      expect(response.body.unauthorizedFiles).toEqual(
        expect.arrayContaining([projectFileId, foreignFileId]),
      );
      expect(processDeleteRequest).not.toHaveBeenCalled();
    });
