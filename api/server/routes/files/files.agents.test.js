...
const { processFileUpload, processAgentFileUpload } = require('~/server/services/Files/process');
...
describe('File Routes - Agent Files Endpoint', () => {
  ...
  describe('POST /files - Agent File Upload Permission Check', () => {
    ...
    it('denies a foreign agent upload even with valid PROJECT EDIT access', async () => {
      await createAgent({
        id: agentCustomId,
        name: 'Foreign Agent',
        provider: 'openai',
    ...
    it('runs assistant uploads through content validation before provider processing', async () => {
      const readFile = jest
        .spyOn(fs, 'readFile')
        .mockResolvedValueOnce(Buffer.from('Contains PRIVATE-1234'));
      ...
      expect(response.status).toBe(400);
      expect(response.body).toEqual(
        expect.objectContaining({ error: 'content_filter_block', field: 'content' }),
      );
