...
const createCurrentSharePreflight = (req) =>
  createShareContentPreflight(req.config?.filters, {
...
if (allowSharedLinks) {
    ...
    async (req, res) => {
      try {
        const share = await getSharedMessages(req.params.shareId, req.shareResourceId, {
          ...
          preflight: createCurrentSharePreflight(req),
        });
        if (!share) {
  ...
  router.get(
    ...
    async (req, res) => {
      try {
        const contentPreflight = createCurrentSharePreflight(req);
...
router.get('/tenant/:shareId', requireJwtAuth, configMiddleware, async (req, res) => {
  try {
    ...
    await createCurrentSharePreflight(req)?.(share);
    res.set('Cache-Control', 'private, no-store');
