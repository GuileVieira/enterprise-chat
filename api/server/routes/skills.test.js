...
describe('Skill routes', () => {
  ...
  describe('POST /api/skills', () => {
    it('creates a skill and grants SKILL_OWNER ACL', async () => {
      ...
      const acl = await runAsSystem(() =>
        AclEntry.findOne({
          resourceType: ResourceType.SKILL,
          resourceId: { $in: [res.body._id, new mongoose.Types.ObjectId(res.body._id)] },
          principalType: PrincipalType.USER,
          principalId: { $in: [testUsers.owner._id, testUsers.owner._id.toString()] },
        }),
      );
