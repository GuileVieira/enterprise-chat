...
router.post('/update', validateConvoAccess, configMiddleware, async (req, res) => {
  ...
  if (
    ...
    (typeof projectId !== 'string' ||
      !isValidProjectFilter(projectId) ||
      projectId === 'unassigned')
  ) {
    return res.status(400).json({ error: 'projectId must be a valid project id or null' });
  }
  ...
  try {
    ...
    if (projectId !== undefined) {
      if (projectId !== null) {
        ...
        if (!allowed) return res.status(403).json({ error: 'Insufficient project permissions' });
      }
