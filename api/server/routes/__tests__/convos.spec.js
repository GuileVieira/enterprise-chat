...
describe('Convos Routes', () => {
  ...
  describe('POST /update project access', () => {
    it('rejects moving a conversation without PROJECT VIEW', async () => {
      const projectId = '3d232aca-6cb4-4d19-a770-85683bfc73fb';
      findProjectById.mockResolvedValue({ _id: 'project-object-id', projectId });
      checkPermission.mockResolvedValue(false);

      const response = await request(app)
        .post('/api/convos/update')
        .send({ arg: { conversationId: 'conv-123', projectId } });

      expect(response.status).toBe(403);
      expect(saveConvo).not.toHaveBeenCalled();
    });
    it('passes an Orqest UUID project filter to the conversation query', async () => {
      const { getConvosByCursor } = require('~/models');
      const projectId = '3d232aca-6cb4-4d19-a770-85683bfc73fb';
      getConvosByCursor.mockResolvedValue({ conversations: [], nextCursor: null });
      const response = await request(app).get('/api/convos').query({ projectId });
      expect(response.status).toBe(200);
      expect(getConvosByCursor).toHaveBeenCalledWith(
        'test-user-123',
