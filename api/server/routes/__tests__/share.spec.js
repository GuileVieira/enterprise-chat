...
describe('share routes', () => {
  ...
  it('authorizes and rechecks current content policy before returning a public artifact', async () => {
    const share = {
      shareId: 'share-123',
    ...
    const response = await request(buildApp({ filters: contentFilters })).get(
      '/api/share/share-123/artifact/report_text%2Fhtml_report_original-message',
    );

    expect(response.status).toBe(200);
    expect(response.headers['cache-control']).toBe('private, no-store');
    expect(mockCanAccessSharedLink).toHaveBeenCalled();
    expect(getSharedMessages).toHaveBeenCalledWith(
      'share-123',
      'resource-123',
      expect.objectContaining({ preflight: expect.any(Function) }),
    );
    expect(mockAssertConversationContentAllowed).toHaveBeenCalled();
    expect(response.body.messages[0].text).toContain('<h1>Shared report</h1>');
    expect(response.body.messages[0].text).not.toContain('private text');
  });

  it('blocks a public artifact when current shared-content policy rejects it', async () => {
    const error = Object.assign(new Error('blocked'), {
      code: 'content_filter_block',
      statusCode: 400,
      body: { error: 'content_filter_block' },
    });
    mockAssertConversationContentAllowed.mockRejectedValueOnce(error);
    ...
    const response = await request(buildApp({ filters: contentFilters })).get(
      '/api/share/share-123/artifact/report',
    );

    expect(response.status).toBe(400);
    expect(response.body).toEqual({ error: 'content_filter_block' });
  });
