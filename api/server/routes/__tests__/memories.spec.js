...
const { createMemory, getAllUserMemories, getUserMemories, updateMemory } = require('~/models');
...
describe('memories routes', () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    getAllUserMemories.mockReset();
    getUserMemories.mockReset();
  });

  describe('GET /', () => {
    it('does not expose stored memory content from projection errors', async () => {
      getAllUserMemories.mockRejectedValue(new Error('PRIVATE-MEMORY-CONTENT'));

      const response = await request(app).get('/api/memories');

      expect(response.status).toBe(500);
      expect(response.body).toEqual({ error: 'Failed to retrieve memories.' });
      expect(JSON.stringify(response.body)).not.toContain('PRIVATE-MEMORY-CONTENT');
    });
  });
  ...
  describe('PATCH /:key', () => {
    ...
    it('trims the new key before validating and renaming', async () => {
      ...
      expect(updateMemory).toHaveBeenCalledWith(
        expect.objectContaining({ originalKey: 'my_key', key: 'new_key' }),
      );
    });

    it('updates the value when the key is unchanged', async () => {
      getUserMemories.mockResolvedValue([{ key: 'my_key', value: 'updated value', tokenCount: 5 }]);
      updateMemory.mockResolvedValue({ ok: true });
      ...
      expect(updateMemory).toHaveBeenCalledWith(
        expect.objectContaining({ userId: 'user-1', originalKey: 'my_key', key: 'my_key' }),
      );
    });
  });
});
