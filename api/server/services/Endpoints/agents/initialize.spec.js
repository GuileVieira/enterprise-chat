...
const mockGetTenantFiles = jest.fn();
...
jest.mock('~/models', () => ({
  ...
  getTenantFiles: (...args) => mockGetTenantFiles(...args),
}));
...
describe('initializeClient — processAgent ACL gate', () => {
  ...
  beforeEach(async () => {
    ...
    mockGetConvo.mockImplementation((userId, conversationId) =>
      mongoose.model('Conversation').findOne({ user: userId, conversationId }).lean(),
    );
  ...
  it('should initialize handoff agent and keep its edge when user has VIEW access', async () => {
    ...
    const req = makeReq();
    req.body.projectId = 'project-handoff';
    mockGetProjectById.mockResolvedValue({
      projectId: 'project-handoff',
      tenantId: 'tenant-handoff',
      fileIds: ['handoff-project-file'],
    });
    mockGetTenantFiles.mockResolvedValue([{ file_id: 'handoff-project-file' }]);
    ...
    mockLoadToolsForExecution.mockResolvedValueOnce({ loadedTools: [], configurable: {} });
    await capturedToolExecuteOptions.loadTools([], AUTHORIZED_ID);
    expect(mockLoadToolsForExecution).toHaveBeenLastCalledWith(
      expect.objectContaining({
        projectId: 'project-handoff',
        projectFileIds: ['handoff-project-file'],
      }),
    );
  });
...
describe('initializeClient — subagent loading', () => {
  ...
  it('preserves subagent tool context for ON_TOOL_EXECUTE (Codex P1 regression guard)', async () => {
    ...
    const req = makeSubagentReq();
    req.body.projectId = 'project-subagent';
    mockGetProjectById.mockResolvedValue({
      projectId: 'project-subagent',
      tenantId: 'tenant-subagent',
      fileIds: ['project-file'],
    });
    mockGetTenantFiles.mockResolvedValue([{ file_id: 'project-file' }]);
    ...
    expect(mockInitializeAgent.mock.calls[1][0]).toEqual(
      expect.objectContaining({
        projectId: 'project-subagent',
    ...
    expect(arg.projectId).toBe('project-subagent');
    expect(arg.projectFileIds).toEqual(['project-file']);
  });
  ...
  it('should prime project files separately from new chat request files', async () => {
    ...
    mockGetTenantFiles.mockResolvedValue([{ file_id: 'project-file' }, { file_id: 'linked-file' }]);
    ...
    expect(mockGetTenantFiles).toHaveBeenCalledWith('tenant-123', {
