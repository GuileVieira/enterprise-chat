...
const initializeClientWithProvider = async ({
  ...
  /** Preserve the normalized owner-scoped fallback for loaders that share this request. */
  ...
  for (const [agentId, config] of agentConfigs) {
    ...
    agentToolContexts.set(agentId, {
      ...buildAgentToolContext({ agent: config, config }),
      projectId,
      projectFileIds,
    });
  }
  ...
  const initializeLoadedSubagent = async ({
    ...
    const validation = await waitForAbort(
      validateAgentModel({ req, res, agent, modelsConfig, logViolation, allowedProviders }),
      context.signal,
    );
    throwIfAborted(context.signal);
