...
async function loadAgentTools({
  ...
  const canUseMCP = hasMCPTools ? await mcpPermissionContext.canUseServers(req.user) : true;
  const canUseTool = await resolveAgentToolPermissions(req, agent.tools, enabledCapabilities);
  const canUseWebSearch = !agent.tools?.includes(Tools.web_search) || canUseTool(Tools.web_search);
  ...
  const _agentTools = normalizeLocalWebTools(agent.tools).filter((tool) => {
    if (tool === Tools.file_search) {
      ...
      includesWebSearch = checkCapability(AgentCapabilities.web_search) && canUseWebSearch;
      return includesWebSearch;
...
async function loadToolsForExecution({
  ...
  const isWebSearchRequested = toolNames.some((name) =>
    [Tools.web_search, Tools.tavily_search_results_json, Tools.fetch_url].includes(name),
  );
  ...
  const webSearchEnabled =
    !isWebSearchRequested || enabledCapabilities?.has(AgentCapabilities.web_search) === true;
  ...
  for (const name of allToolNamesToLoad) {
    if (
      (name === Tools.tavily_search_results_json || name === Tools.fetch_url) &&
      !webSearchEnabled
    ) {
      logger.warn(
  ...
  if (regularToolNames.length > 0) {
    const includesWebSearch = regularToolNames.some((name) =>
      [Tools.web_search, Tools.tavily_search_results_json, Tools.fetch_url].includes(name),
    );
