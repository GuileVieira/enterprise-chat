...
describe('setOpenIDAuthTokens', () => {
  ...
  describe('session token storage', () => {
    ...
    it('does not use an access token when a refreshed id token is expired', () => {
      const expiredIdToken = jwt.sign(
        { sub: 'user-123', exp: Math.floor(Date.now() / 1000) - 60 },
        'idp-signing-secret',
      );
      ...
      const req = mockRequest({ openidTokens: { idToken: expiredIdToken } });
      expect(setOpenIDAuthTokens(tokenset, req, mockResponse(), 'user-123')).toBeUndefined();
    });
    ...
    it('still prefers an id_token whose expiry cannot be read', () => {
      ...
      expect(setOpenIDAuthTokens(tokenset, mockRequest(), mockResponse(), 'user-123')).toBe(
