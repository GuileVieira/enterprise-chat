...
jest.mock('@librechat/api', () => {
  ...
  return {
    ContentFilterError: class ContentFilterError extends Error {
      constructor(finding) {
        super('content filter block');
        this.body = finding;
      }
    },
...
describe('processFileUpload project linking', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    mockRes.status.mockReturnThis();
    mockRes.json.mockReturnValue({});
  ...
  test('persists and links an assistant message file uploaded to a project', async () => {
    const handleFileUpload = jest.fn().mockResolvedValue({
    ...
    getStrategyFunctions.mockReturnValue({ handleFileUpload });
    db.createFile.mockResolvedValueOnce({ file_id: 'assistant-file-id' });
    const req = makeReq({
      body: { projectId: 'project-123', model: 'gpt-4o' },
    });
    ...
    expect(db.createFile).toHaveBeenCalledWith(
      expect.objectContaining({
        file_id: 'assistant-file-id',
        projectId: 'project-123',
      }),
      true,
    );
    expect(db.addProjectFileId).toHaveBeenCalledWith('project-123', 'assistant-file-id');
  });
});
...
describe('processAgentFileUpload', () => {
  beforeEach(() => {
    ...
    getStrategyFunctions.mockReturnValue({
      handleFileUpload: jest
        .fn()
        .mockResolvedValue({ text: 'OCR text from image', bytes: 42, filepath: 'ocr://result' }),
  ...
  describe('content filtering for extracted context', () => {
    ...
    it('blocks extracted document text before agent-resource and file persistence', async () => {
      getStrategyFunctions.mockReturnValue({
        handleFileUpload: jest
          .fn()
          .mockResolvedValue({ text: 'extracted text', bytes: 42, filepath: 'text://result' }),
      });
  ...
  describe('image file_search uploads', () => {
    ...
    test('stores the image, extracts OCR text, and indexes the saved image record for file_search', async () => {
      ...
      const sseStream = { sendData: jest.fn() };
      await processAgentFileUpload({
        req,
        res: mockRes,
        sseStream,
      ...
      expect(db.createFile).toHaveBeenCalledWith(
        expect.objectContaining({
          ...
          metadata: expect.objectContaining({
            imageRag: expect.objectContaining({ kind: 'ocr_text' }),
          }),
        }),
        true,
      );
      ...
      expect(uploadVectors).toHaveBeenCalledWith(
        expect.objectContaining({
          ...
          file: expect.objectContaining({
            originalname: 'landing.png.ocr.txt',
            mimetype: 'text/plain',
          }),
        }),
      );
      ...
      expect(openRouterBody.messages[1].content[1].image_url.url).toBe(
    ...
    test('links only the original image to the project for file_search', async () => {
      mergeFileConfig.mockReturnValue(makeFileConfig({ ocrSupportedMimeTypes: ['image/png'] }));
      const req = makeReq({
        mimetype: 'image/png',
        ocrConfig: { strategy: FileSources.mistral_ocr },
    ...
    test('fails closed before vector upload when derived OCR text violates policy', async () => {
      mergeFileConfig.mockReturnValue(makeFileConfig({ ocrSupportedMimeTypes: ['image/png'] }));
      const req = makeReq({
        mimetype: 'image/png',
        ocrConfig: { strategy: FileSources.mistral_ocr },
      });
      const policyError = Object.assign(new Error('blocked'), {
        body: { error: 'content_filter_block', field: 'extracted_text' },
      });
...
describe('processImageFile', () => {
  ...
  test('persists and links a normal image uploaded to a project', async () => {
    const handleImageUpload = jest.fn().mockResolvedValue({
      filepath: '/images/user-123/project.webp',
      bytes: 256,
      width: 100,
      height: 80,
    });
    getStrategyFunctions.mockReturnValue({ handleImageUpload });
    db.createFile.mockResolvedValueOnce({ file_id: 'project-image-id' });
    const req = makeReq({ mimetype: 'image/png', body: { projectId: 'project-123' } });

    await processImageFile({
      req,
      res: mockRes,
      metadata: { file_id: 'project-image-id', endpoint: EModelEndpoint.agents },
    });
