...
const processImageFileSearchUpload = async ({ req, res, metadata, sseStream }) => {
  const { file } = req;
  const appConfig = req.config;
  ...
  try {
    ...
    assertExtractedTextInspectable({ filters: appConfig?.filters, text: extractedText });
    if (hasActiveFileFieldPolicy(appConfig?.filters, ['extracted_text'])) {
      const finding = inspectContent(extractFileContent({ extractedText }), {
    ...
    return sendUploadSuccess(res, sseStream, 'Agent file uploaded and processed successfully', {
      ...indexedImageFile,
      imageRag: {
        textBytes,
        source: ocrSource,
    ...
    return sendUploadSuccess(res, sseStream, 'Image uploaded, but OCR RAG processing failed', {
      ...imageFile,
    });
  }
};
...
const processAgentFileUpload = async ({ req, res, metadata, sseStream }) => {
  ...
  const usesImageOcrForFileSearch =
  ...
  if (
    ...
    !usesImageOcrForFileSearch &&
    !canToolResourceConsume(effectiveToolResource, file.mimetype)
  ) {
    throw new Error(
  ...
  if (effectiveToolResource === EToolResources.file_search) {
    ...
    if (isImage) {
      return await processImageFileSearchUpload({ req, res, metadata, sseStream });
    }
