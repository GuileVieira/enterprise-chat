...
const refreshController = async (req, res) => {
  ...
  if (token_provider === 'openid' && isEnabled(process.env.OPENID_REUSE_TOKENS)) {
    ...
    try {
      ...
      return await runInUserTenant(refreshUser, async () => {
        ...
        let refreshedAppToken;
        try {
          refreshedAppToken = await sendOpenIDAuthResponse({
            tokenset,
            user,
          ...
          if (error?.message !== 'OpenID refresh returned no application authentication token') {
            throw error;
          }
        }
        const token = refreshedAppToken || (await generateOpenIDFallbackAppToken(user));
  ...
  try {
    ...
    return tenantStorage.run({ tenantId: userTenantId }, async () => {
