...
const chatV2 = async (req, res) => {
  ...
  try {
    ...
    projectInstructions = projectContext.projectInstructions ?? '';
    projectMemories = projectContext.projectMemories ?? '';
    ...
    const getRequestFileIds = async () => {
      ...
      if (files.length || thread_file_ids.length || projectFileIds.length) {
        ...
        const attachedRequestFileIds = new Set([...file_ids, ...thread_file_ids]);
        for (const file_id of projectFileIds) {
          if (attachedRequestFileIds.has(file_id)) {
            continue;
          }
          attachedRequestFileIds.add(file_id);
          if (!userMessage.attachments) {
            userMessage.attachments = [];
          }
          userMessage.attachments.push({
