...
const deleteUserData = async (req, user) => {
  ...
  await db.deleteSchedulesByUser(userId);
  return db.deleteUserById(userId);
};

const deleteUserDataWithLifecycle = async (req, user) => {
  const userId = user.id ?? user._id?.toString();
  const tenantId = user.tenantId || undefined;
  const scheduleSuspensionToken = randomUUID();
  ...
  try {
    ...
    const activeRuns = await GenerationJobManager.getAccountCleanupJobIdsForUser(userId, tenantId);
    const jobs = await Promise.all(
      activeRuns.map(async (streamId) => ({
        streamId,
    ...
    const appConfig = await getAppConfig({ role: user.role, userId, tenantId });
    ...
    const result = await deleteUserData({ ...req, config: appConfig }, user);
    userDeleted = result?.deletedCount === 1;
    return result;
  } finally {
    if (!userDeleted) {
      try {
        await restoreUserSchedulesFromDeletion(userId, scheduleSuspensionToken);
      } catch (error) {
        logger.error(
          `[deleteUserDataWithLifecycle] Failed to restore schedules for ${userId} under ${scheduleSuspensionToken}`,
          error,
        );
      }
    }
