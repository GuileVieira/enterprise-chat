...
describe('deleteUserData', () => {
  ...
  it('uses the target user config while deleting target-owned files', async () => {
    const targetConfig = { fileStrategy: 'target-storage' };
    const user = {
    ...
    mockGetAppConfig.mockResolvedValueOnce(targetConfig);

    await deleteUserDataWithLifecycle({ user: { id: 'admin-id' }, config: {} }, user);
    ...
    expect(mockProcessDeleteRequest).toHaveBeenCalledWith(
      expect.objectContaining({
        req: expect.objectContaining({ config: targetConfig }),
      }),
    );
  });
});
