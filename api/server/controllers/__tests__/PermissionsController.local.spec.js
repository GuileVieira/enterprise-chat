...
jest.mock('~/server/services/PermissionService', () => ({
  ...
  restoreInsightsPermissionChanges: jest.fn(),
...
describe('PermissionsController', () => {
  ...
  describe('updateResourcePermissions — favorites cleanup', () => {
    ...
    it('passes tenant principals through to the permission service', async () => {
      ...
      const req = createMockReq({
        ...
        user: { id: 'user-1', role: SystemRoles.OWNER, tenantId: 'tenant-2' },
      });
      const res = createMockRes();
    ...
    it('rejects foreign tenant agent grants from non-admin users', async () => {
      const req = createMockReq({
        body: {
          updated: [
            {
        ...
        user: { id: 'owner-1', role: SystemRoles.OWNER, tenantId: 'tenant-a' },
      });
      const res = createMockRes();
    ...
    it('rejects foreign tenant agent revokes from non-admin users', async () => {
      const req = createMockReq({
        body: {
          updated: [],
          removed: [{ type: PrincipalType.TENANT, id: 'tenant-b' }],
        },
        user: { id: 'owner-1', role: SystemRoles.OWNER, tenantId: 'tenant-a' },
      });
      const res = createMockRes();
  ...
  describe('searchPrincipals tenant scoping', () => {
