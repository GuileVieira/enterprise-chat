...
const filterAuthorizedTools = async ({
  ...
  const tenantFunctionIds = new Set(tenantFunctions.map(({ id }) => id));
...
const createAgentHandler = async (req, res) => {
  try {
    ...
    const agent = await createAgentInRequestTenant(req, () => db.createAgent(agentData));
    ...
    try {
      ...
      await Promise.all(permissionGrants);
      logger.debug(
  ...
  try {
    ...
    if (requestedMCPTools.length > 0 || (hasToolUpdate && existingMCPTools.length > 0)) {
      ...
      if (!(await mcpPermissionContext.canUseServers(req.user))) {
        ...
        const newTools = updateData.tools.filter((tool) => !existingToolSet.has(tool));
        ...
        if (newTools.length > 0) {
          ...
          const rejectedSet = new Set(newTools.filter((tool) => !approvedNew.includes(tool)));
...
const duplicateAgentHandler = async (req, res) => {
  ...
  try {
    ...
    let newAgent;
    try {
      newAgent = await createAgentInRequestTenant(req, () => db.createAgent(newAgentData));
    } catch (error) {
    ...
    try {
      await Promise.all([
        ...
        ...(req.user.role === SystemRoles.OWNER && req.user.tenantId
          ? [
              grantPermission({
                principalType: PrincipalType.TENANT,
                principalId: req.user.tenantId,
...
const getListAgentsHandler = async (req, res) => {
  try {
     ...
     */
    let needsEditableLookup = !hasEditBit(requiredPermission);
    // Base filter
    const filter = {};
    ...
    needsEditableLookup = needsEditableLookup && !canManageAgents;
    ...
    const [
      ...
      canManageAgents
        ? db.getAgents({}).then((agents) => agents.map((agent) => agent._id))
        : findAccessibleResources({
            userId,
            role: req.user.role,
            idOnTheSource,
