...
class AgentClient extends BaseClient {
  ...
  async buildMessages(messages, parentMessageId, _buildOptions, opts) {
    ...
    const prepareRuntimeAgent = async (
      ...
      if (agentId === this.options.agent.id || memoryAgentEnabled || agentHasMemory) {
        ...
        const memoryText = agentHasMemory
          ? [partitionMemories?.withKeys, partitionMemories?.sharedWithoutKeys]
              .filter(Boolean)
              .join('\n\n')
          : partitionMemories?.withoutKeys;
        const agentMemoryContext = buildMemoryContext(memoryText);
  ...
  async useMemory() {
    ...
    let sharedMemoryText = '';
    try {
      const ownMemories = await getRequestMemories({
        req: this.options.req,
        userId,
      ...
      existingMemories = {
        withKeys: ownMemories.withKeys,
        withoutKeys: ownMemories.withoutKeys,
      };
    } catch (error) {
      logger.error(
    ...
    try {
      const memoryUserIds = await getTenantMemoryUserIds(user, { findUsers: db.findUsers });
      const sharedMemories = await Promise.all(
        memoryUserIds
          .filter((memoryUserId) => memoryUserId !== userId)
          .map((memoryUserId) =>
            db.getFormattedMemories({ userId: memoryUserId, agentId: memoryAgentId }),
          ),
      );
      sharedMemoryText = sharedMemories
      ...
      if (sharedMemoryText) {
        existingMemories = {
          ...existingMemories,
          sharedWithoutKeys: sharedMemoryText,
          withoutKeys: [existingMemories?.withoutKeys, sharedMemoryText]
            .filter(Boolean)
            .join('\n\n'),
        };
      }
     ...
     *  loader already made. */
    let memoryToolGrants;
    let projectContext;
    try {
      memoryToolGrants = memoryCodeEnabled
        ...
        : null;
      projectContext =
        this.options.projectId || this.conversationId
          ? await loadProjectContext({
              req: this.options.req,
              conversationId: this.conversationId,
              projectId: this.options.projectId,
            })
          : { projectId: undefined, projectFileIds: [] };
    } catch (error) {
      logger.error(
        '[AgentClient] Memory extraction initialization failed',
        getSafeErrorMetadata(error),
      );
      return existingMemories;
    }
      ...
      agent = await initializeAgent(
        {
          ...
          statefulSessionsAvailable: memoryCapabilities.has(
    ...
    return {
      withKeys: withKeys ?? existingMemories?.withKeys,
      withoutKeys:
        [withoutKeys, sharedMemoryText].filter(Boolean).join('\n\n') ||
        existingMemories?.withoutKeys,
      ...(sharedMemoryText ? { sharedWithoutKeys: sharedMemoryText } : {}),
    };
  }
