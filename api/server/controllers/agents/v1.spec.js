...
describe('Agent Controllers - Mass Assignment Protection', () => {
  ...
  describe('createAgentHandler', () => {
    test('tenantless ADMIN creation ignores ambient tenant storage', async () => {
      mockReq.user.role = SystemRoles.ADMIN;
      mockReq.body = { name: 'Global Agent', provider: 'openai', model: 'gpt-4' };

      await tenantStorage.run({ tenantId: 'ambient-tenant' }, () =>
        createAgentHandler(mockReq, mockRes),
      );

      expect(mockRes.status).toHaveBeenCalledWith(201);
      const created = mockRes.json.mock.calls[0][0];
      expect(created.tenantId).toBeUndefined();
      const storedAgent = await Agent.findOne({ id: created.id }).lean();
      expect(storedAgent.tenantId).toBeUndefined();
    });

    test('authorizes an active tenant function during creation', async () => {
      jest.spyOn(db, 'getTenantFunctions').mockResolvedValueOnce([{ id: 'tenant-function' }]);
      mockReq.user.tenantId = 'tenant-1';
      mockReq.body = {
        name: 'Tenant Function Agent',
        provider: 'openai',
        model: 'gpt-4',
        tools: ['tenant-function'],
      };

      await createAgentHandler(mockReq, mockRes);

      expect(mockRes.status).toHaveBeenCalledWith(201);
      expect(mockRes.json.mock.calls[0][0].tools).toEqual(['tenant-function']);
      expect(db.getTenantFunctions).toHaveBeenCalledWith({ tenantId: 'tenant-1', isActive: true });
    });
  ...
  describe('tenant function tool authorization', () => {
    it('keeps active tenant functions and rejects foreign function ids', async () => {
      const db = require('~/models');
      jest.spyOn(db, 'getTenantFunctions').mockResolvedValueOnce([{ id: 'tenant-function' }]);

      await expect(
        filterAuthorizedTools({
          tools: ['tenant-function', 'foreign-function'],
          userId: mockReq.user.id,
          availableTools: {},
          tenantId: 'tenant-1',
        }),
      ).resolves.toEqual(['tenant-function']);
      expect(db.getTenantFunctions).toHaveBeenCalledWith({ tenantId: 'tenant-1', isActive: true });
    });
  });
  ...
  describe('updateAgentHandler', () => {
    ...
    test('authorizes a newly selected active tenant function', async () => {
      const db = require('~/models');
      jest.spyOn(db, 'getTenantFunctions').mockResolvedValueOnce([{ id: 'tenant-function' }]);
      mockReq.user.id = existingAgentAuthorId.toString();
      mockReq.user.tenantId = 'tenant-1';
      mockReq.params.id = existingAgentId;
      mockReq.body = { tools: ['tenant-function'] };
  ...
  describe('tool_resources ownership pruning in alternate write paths', () => {
    ...
    test('duplicateAgentHandler grants the OWNER tenant viewer access', async () => {
      const sourceAuthorId = new mongoose.Types.ObjectId();
      const cloneAuthorId = new mongoose.Types.ObjectId();
      const sourceAgent = await Agent.create({
        id: `agent_${uuidv4()}`,
      ...
      const { agent } = mockRes.json.mock.calls[0][0];
      expect(agent.tenantId).toBe('tenant-owner');
      expect(grantPermission).toHaveBeenCalledWith(
        expect.objectContaining({
          principalType: PrincipalType.TENANT,
          principalId: 'tenant-owner',
    ...
    test('revertAgentVersionHandler restores an active tenant function', async () => {
      const agentAuthorId = new mongoose.Types.ObjectId();
      const agent = await Agent.create({
        id: `agent_${uuidv4()}`,
        name: 'Current Agent',
  ...
  describe('getListAgentsHandler - Security Tests', () => {
    ...
    test('management capability lists all agents without exposing editable skill config', async () => {
      const hiddenSkillId = new mongoose.Types.ObjectId();
      await Agent.updateOne({ id: agentA1.id }, { skills_enabled: true, skills: [hiddenSkillId] });
      ...
      expect(hasCapability).toHaveBeenCalledWith(
        mockReq.user,
        require('@librechat/data-schemas').SystemCapabilities.MANAGE_AGENTS,
      );
      const response = mockRes.json.mock.calls[0][0];
      expect(response.data).toHaveLength(4);
      expect(response.data.every((agent) => agent.isEditable === true)).toBe(true);
      const managedAgent = response.data.find((agent) => agent.id === agentA1.id);
      expect(managedAgent.skills).toBeUndefined();
    });
