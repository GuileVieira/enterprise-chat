...
describe('AgentClient - titleConvo', () => {
  ...
  describe('buildMessages with request and agent-scoped context attachments', () => {
    ...
    it('inspects read-only tenant owner memory before adding it to model context', async () => {
      const privateOwnerMemory = 'PRIVATE-OWNER-MEMORY';
      mockReq.config.filters = {
        memories: {
          pii: {
            fields: ['value'],
            starterPatterns: [],
            customPatterns: [
              { id: 'private-owner', label: 'private owner value', regex: privateOwnerMemory },
            ],
          },
        },
      };
      require('~/models').getUserMemories.mockResolvedValue([]);
      client.useMemory.mockResolvedValue({
        withKeys: 'personal-key: safe personal memory',
        withoutKeys: `safe personal memory\n\n${privateOwnerMemory}`,
      });
  ...
  describe('buildMessages - memory context for parallel agents', () => {
    ...
    it('adds tenant owner memory to inline context without exposing owner keys', async () => {
      mockAgent.memoryToolsRegistered = true;
      client.useMemory = jest.fn().mockResolvedValue({
        withKeys: 'personal-key: personal fact',
        withoutKeys: 'personal fact\n\nshared owner fact',
        sharedWithoutKeys: 'shared owner fact',
      });
      ...
      expect(client.options.agent.additional_instructions).toContain('personal-key: personal fact');
      expect(client.options.agent.additional_instructions).toContain('shared owner fact');
      expect(client.options.agent.additional_instructions).not.toContain('owner-secret-key');
    });
  ...
  describe('useMemory method - prelimAgent assignment', () => {
    ...
    it.each(['missing', 'null', 'initialize-error', 'processor-error'])(
      ...
      async (failure) => {
        ...
        client = new AgentClient(mockOptions);
        expect(await client.useMemory()).toEqual(
          expect.objectContaining({ withoutKeys: 'preserved context' }),
        );
    ...
    it('keeps tenant owner memory read-only when extraction is enabled', async () => {
      mockReq.user.tenantId = 'tenant-1';
      require('~/models').findUsers.mockResolvedValueOnce([{ _id: 'owner-456' }]);
      mockGetFormattedMemories.mockImplementation(({ userId }) =>
        Promise.resolve(
          userId === 'owner-456'
            ? { withKeys: 'owner-secret-key: shared fact', withoutKeys: 'shared fact' }
            : { withKeys: 'personal-key: personal fact', withoutKeys: 'personal fact' },
        ),
      );
      mockCheckAccess.mockResolvedValue(true);
      mockInitializeAgent.mockResolvedValue(mockAgent);
      mockCreateMemoryProcessor.mockResolvedValue(['refreshed personal fact', jest.fn()]);
      ...
      expect(result).toEqual({
        withKeys: 'personal-key: personal fact',
        withoutKeys: 'refreshed personal fact\n\nshared fact',
        sharedWithoutKeys: 'shared fact',
      });
      expect(result.withKeys).not.toContain('owner-secret-key');
    });

    it('preserves personal memory when tenant owner discovery fails', async () => {
      mockReq.user.tenantId = 'tenant-1';
      mockReq.config.memory = { personalize: true };
      require('~/models').findUsers.mockRejectedValueOnce(new Error('owner lookup failed'));
      mockGetFormattedMemories.mockResolvedValue({
        withKeys: 'personal-key: personal fact',
        withoutKeys: 'personal fact',
      });
      ...
      await expect(client.useMemory()).resolves.toEqual({
        withKeys: 'personal-key: personal fact',
        withoutKeys: 'personal fact',
      });
    });
